...
\item Estimate hyperparameter, $\pi=(\eta,\sigma,c)$: 
  ...
  \begin{enumerate}
  \item Use {\tt edgeR} to obtain $\hat{c}$ and $\hat{\theta}_g = (\hat{\phi}_g,\hat{\alpha}_g,\hat{\delta}_g,\hat{\psi}_g)$.
...
\begin{itemize}
\item {\tt edgeR} (not fair), 
\item {\tt baySeq} (not fair), 
...
\[ 
\eta_\alpha^{\mbox{truth}} \equiv \frac{1}{G} \sum_{g=1}^G \alpha_g^{\mbox{truth}} \pause \qquad 
\sigma_\alpha^{2\mbox{truth}} \equiv \frac{1}{G} \sum_{g=1}^G \left( \alpha_g^{\mbox{truth}} - \eta_\alpha^{\mbox{truth}}\right)^{2}
\]
...
\[ 
\left| \delta_g^{edgeR}\right| \le \left| \alpha_g^{edgeR}\right| 
\]
...
\section{Disease outbreaks}
\begin{frame}
\url{https://michaud.shinyapps.io/CDCPlot/}
\end{frame}
