...
model {
  ...
  count ~ neg_binomial_2_log(theta, 1/psi);
}
"
...
M = 1000
mu_theta = 0; tau_theta=1, mu_psi = 0; mu_psi = 1 # Initial values
n_iter = 10
...
for (i in 1:n_iter) {
  # Run MCMC using rstan
  draws = ddply(d, .(gene), function(x) {
    s = sampling(nb_m, data = list(n = length(x$y), y=x$y, 
                                   mu_theta=mu_theta, tau_psi=tau_theta, 
...
<<dependson=c('normal_hierarchical_mean_em','normal_hierarchical_mean_mcem'), echo=FALSE>>=
ggplot(keep, id.var=c("iteration"), variable.name="parameter"), 
       aes(iteration, value, color=method)) +
  geom_point() + 
  facet_wrap(~parameter, scales="free_y") +
  geom_hline(data=melt(truth, variable.name='parameter'), aes(yintercept=value))
