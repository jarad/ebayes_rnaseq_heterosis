...
% WML: change: "edgeR was originally designed to..."

{\tt edgeR} was originally designed to test for differential expression between two varieties \citep{robinson2007moderated, robinson2010edgeR}, and we used this capability to construct a statistic for each gene. If the hybrid's mean expression does not differ from a parent, then the hybrid does not exhibit heterosis. We can calculate two pvalues: one for the null hypothesis that the hybrid's mean expression is the same as parent 1 and the second for the null hypothesis that the hybrid's mean expression is the same as parent 2. But, even if both pvalues are small, the hybrid's mean expression could be between the two parents, i.e. mid-parent heterosis, and therefore not be exhibiting extreme heterosis. We construct a statistic that is 0 if the estimate of mean expression 
