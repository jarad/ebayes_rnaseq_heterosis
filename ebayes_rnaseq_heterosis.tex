...
% WML: general comments:
%
% I rewrote the paragraphs on edgeR, baySeq, and ShrinkBayes. Please let me know what you think. Other than that, I modified the actual text in some places, but most of my feedback is in LaTeX comment form.
%
% Notation: there are some discrepancies, mainly between the phi-alpha-delta parameterization in ShrinkBayes and other usages of the greek letters delta and phi. To avoid this, you could use entirely different letters in that one ShrinkBayes paragraph. Also, you use the "f" subscript in and F to denote the number of genes when I think you're trying to stick to g and G, respectively. I didn't correct this because I wasn't exactly sure if you wanted to switch to f and F.
%
% The bibtex file didn't compile for me. I think an easy fix is to rename jarad.bib => ebayes_rnaseq_heterosis.bib and change \bibliography{jarad} => \bibliography{ebayes_rnaseq_heterosis}, but I did not make this change.
%
% Please let me know if/when you would like another iteration of edits from me.
...
% WML: Maybe it would be good to quickly clarify what a formal heterosis RNA-seq experiment is. 
...
% WML: "borrowing information" is intuitive and informal. I agree that the term should be used, but there might be a slightly more appropriate use. Not a big deal. 

\subsection{Hierarchical model for RNAseq counts}
\label{s:model}

% WML: We were debating whether to use g for gene or f for feature, and either choice would work. But we use g, we should probably also use G to denote the number of genes, rather than F. 
...
% WML: Are the sequencing depths c_{vi} or \delta_{vi}? I think c_{vi} is better because \delta is used in the phi-alpha-delta parameterization of gene expression effects.
...
% WML: Is \mu_g the vector of \mu_{gv} values? If so, please clarify.
...
% WML: I assume the "hierarchical means" are the hyperparameters in the \eta vector and the "marginally uniform correlations" are the ones derived from the off-diagonal covariance elements of \Sigma. Small clarifications might be helpful in passing. 

\subsection{Empirical Bayes}
\label{s:ebayes}

% WML: The use of \phi in this section conflicts with the phi-alpha-delta parameterization in other heterosis models. Maybe you could replace phi, alpha, and delta by other letters in the ShrinkBayes section to avoid this conflict.
...
% WML: \ref{s:intro} or \ref{s:heterosis}?

% WML: the first two sentences are unclear. I would delete the first sentence altogether and compare "varieties" to treatment groups or blocks for the sake of clarity. 
...
% WML: For mid-parent heterosis, you need \mu_{g3} \ne \frac{\mu_{g1} + \mu_{g2}}{2}. I don't know whether you need \mu_{g1}\le \mu_{g3}\le \mu_{g2} or \mu_{g2}\le \mu_{g3}\le \mu_{g1}, and I have doubts. It might be good to check with Dan on this. 
...
% WML: You may want to clarify that the genes in the list are ranked by the strength of EH and then justify the lists's utility for geneticists and in the comparison study to follow. This could be a good segue.
...
% WML: Please clarify that {\tt edgeR}, {\tt baySeq}, and {\tt ShrinkBayes} are R packages. You may even want to cite R somewhere.
...
% WML: Sentences 2-4 in the paragraph below are redundant with sentence 1. I commented out the paragraph below and inserted a modified one afterwards
...
% WML: "... provide a statistic representing the plausibility of heterosis, i.e. higher values indicating higher plausibility." => "...provide statistics to measure the strength of heterosis of each gene."

% WML: I rewrote the paragraph on edgeR.
...
% WML: I rewrote the baySeq paragraph.
...
% WML: your use of "following" in this last sentence is not clear to me.

% WML: Here, you use the phi-alpha-delta parameterization. You already defined phi and delta in previous sections. To avoid a conflict, you could easily change the notation here. It doesn't even have to be super intuitive because you only use it in this one paragraph.
...
% WML: You might want want to cite a more detailed explanation of the construction of ROC curves. My master's project (Landau and Liu (2013) at http://will-landau.com/research.html) is one example, but there are other resources.
...
% WML: I'm so glad you went ahead and added Andy's name! I couldn't have done it without him.
