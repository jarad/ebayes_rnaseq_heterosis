...
The supplementary materials include these plain text files: the maize experiment data ({\tt data.csv}), a script to run our method ({\tt script.R}) on these data, a script to create the figures ({\tt figs.R}), two functions ({\tt get\_hyperparameters.R} and {\tt single\_gene\_analysis.R}), and the \Stan{} model ({\tt model.stan}).  
