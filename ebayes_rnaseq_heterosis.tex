% WML: Throughout, I made several scattered grammatical and clarity fixes without documentation since I couldn't contribute much else. I have far fewer comments this time around, so I just grouped them all together here at the top.
%
% In section 2.1, your first mention of the sequencing depth terms uses c_{vi}, but those sequencing depths are \gamma_{vi} everywhere else. I didn't change this.
%
% I still can't compile jarad.bib. The console tells me that it couldn't find jarad.aux.
%
% Please decide between "over-dispersion" and "overdispersion".
%
% The definition of MPH is unchanged, and I wonder if we even need to include it.
%
% From sections 3 onward, I think you should stick to the present tense.
%
% First paragraph of the discussion: "We followed their approach modified slightly to allow for any number of varieties and therefore even more complex hypotheses." This sentence suggests to me that the paper actually took this additional step, which it didn't. I think that we should be clear that we only made this future goal more reachable. 
%
% The short paragraph in the Supplementary Materials section was unclear. I tried to clear it up, but I may have changed the original intended meaning.
%
% WML: End of comments.
...
heterosis of traits, such as plant height or grain yield. Because of the unwieldy composite null hypothesis in equation \eqref{e:hypotheses}, existing methods for identifying gene expression heterosis based on RNAseq data are not directly applicable for detecting heterosis genes. \cite{ji2014estimation} introduced an empirical Bayes approach based on a hierarchical model for microarray data. We followed their approach modified slightly to allow for any number of varieties and therefore even more complex hypotheses. We presented an empirical Bayes method to obtain MAP estimates for hyperparameters, followed by an MCMC to estimate gene-specific parameters that can be accomplished efficiently via parallelization. This methodology can be conveniently implemented in the statistical software, {\tt Stan}, which also allows for far more complex models. The empirical Bayes posteriors can be used to estimate posterior probabilities of high and low parent heterosis. 
