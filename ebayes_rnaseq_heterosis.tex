...
\label{f:auc}
\end{figure}
Similar to the single ROC curve, the eBayes and \ShrinkBayes{} methods appear to outperform the other method. With 4 replicates per variety, there does not appear to be much of a difference between \ShrinkBayes{} and the eBayes approaches, but as the number of replicates increases the eBayes approaches appear to improve relative to \ShrinkBayes{}. Two difference exist between the \ShrinkBayes{} and eBayes approaches that could explain the difference in AUC: 1) \ShrinkBayes{} utilizes a different empirical Bayes approach for estimating both the hyperparameters and the gene-specific parameters and 2) the measure of overdominance is slightly different due to INLA not providing a full posterior. 
