...
In the maize context that motivates this work, we are interested in heterosis, either low-parent heterosis (LPH) or high-parent heterosis (HPH), in gene expression. 
...
For a particular gene, the truth was determined via the estimated values for $\alpha_g$ and $\delta_g$. Specifically, if $|\hat{\delta}_g| >|\hat{\alpha}_g|$ the gene was considered to have heterosis. For many heterosis genes, the value of $|\hat{\delta}_g|$ was only slightly larger than $|\hat{\alpha}_g|$. Thus there are many genes in these simulated data sets that are considered to have heterosis, but whose signal in the simulations will be extremely small. Conversely, there are many non-heterosis genes whose value of $|\hat{\delta}_g|$ was only slightly smaller than $|\hat{\alpha}_g|$, but whose simulated data will look similar to many heterosis genes. Therefore, we expect it will be difficult to accurately identify heterosis genes, but believe this level of difficulty is representative of real applications.
