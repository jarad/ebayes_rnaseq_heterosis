...
The supplementary materials include the maize experiment data ({\tt data.csv}), a script to run our method ({\tt script.R}), and {\tt Stan} models: all genes independent (\verb@\tt model_ind.txt\verb@) and covariance (\verb@model_cov.txt\verb@) models used in obtaining MAP estimates and single gene independent (\verb@\tt sg_model_ind.txt\verb@) and covariance (\verb@sg_model_cov.txt\verb@) models used in performing the MCMC on single genes. 
