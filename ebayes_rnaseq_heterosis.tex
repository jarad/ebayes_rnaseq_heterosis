...
% % & \multicolumn{1}{c|}{Empirical} & \multicolumn{3}{c}{Number of replicates}\\
% % Parameter & value & 4 & 8 & 16 \\
% % \hline
% % $\eta_\phi$ & \phantom{-}4.6\phantom{00} & \phantom{-}4.7\phantom{00} & \phantom{-}4.7\phantom{00}  & \phantom{-}4.8\phantom{00}  \\
% % $\eta_\alpha$ & -0.028 & -0.023  & -0.018 & -0.014  \\
% % $\eta_\delta$ & \phantom{-}0.070 & \phantom{-}0.046 & \phantom{-}0.031  & \phantom{-}0.022   \\
% % $\eta_\psi$ & -2.4\phantom{00} & -2.3\phantom{00}  &  -2.3\phantom{00}  & -2.4\phantom{00} \\
% % \hline
% % $\sigma_\phi$ & \phantom{-}1.8\phantom{00} & \phantom{-}1.8\phantom{00}  & \phantom{-}1.8\phantom{00}  & \phantom{-}1.7\phantom{00} \\
% % $\sigma_\alpha$ & \phantom{-}0.45\phantom{0} & \phantom{-}0.46\phantom{0}  & \phantom{-}0.40\phantom{0}  & \phantom{-}0.36\phantom{0}  \\
% % $\sigma_\delta$ & \phantom{-}0.31\phantom{0} & \phantom{-}0.39\phantom{0}  & \phantom{-}0.32\phantom{0}  & \phantom{-}0.27\phantom{0}  \\
% % $\sigma_\psi$ & \phantom{-}0.59\phantom{0} & \phantom{-}0.40\phantom{0} & \phantom{-}0.49\phantom{0} & \phantom{-}0.54\phantom{0} \\
% % \hline
% % \end{tabular}
% % \end{table}
% Table \ref{t:hyperparameter} provides the computed means and standard deviations from the gene-specific parameter values, called empirical values, and compares these values against the mean of estimated hyperparameters across the 10 simulations within replicate number. For the mean parameters, there is reasonable agreement amongst the different numbers of replicates with a somewhat high estimate for the mean of the overdispersion parameter $(\eta_\psi)$ and, perhaps, a trend for the mean of the hybrid effect ($\eta_\delta$), although we would not have expected this. For the standard deviation parameters, $\sigma_\phi$ is consistent across the replicate numbers, $\sigma_\alpha$ and $\sigma_\delta$ mean estimates are decreasing toward the empirical values as the number of replicates increases, and $\sigma_\phi$ is increasing toward the empirical value as the number of replicates increases. %We expect this pattern for $\sigma_\alpha$ and $\sigma_\delta$ since these are based on moment matching of independent gene-specific parameter estimates. For low numbers of replicates, these independent estimates will have relatively large sampling variability that will cause a bias toward larger values for the moment-matched standard deviation estimates.
