...
We assume a hierarchical model for the gene-specific mean parameters and overdispersion parameters. Initially, we assume the parental averages, half-parently differences, hybrid effect, and overdispersion parameters follow normal distributions, i.e.
\[ 
...
For the methods in Sections \ref{s:ebayes}, \ref{s:shrinkbayes}, and \ref{s:alternative}, we sorted genes according to the computed measure of the strength of evidence for overdominance. From these sorted lists, we constructed receiver-operating characteristic (ROC) curves to evaluate the ability of these methods to distinguish genes with overdominance, as defined in Section \ref{s:sim_data}, 
% From Dan, "why this citation?"
from those without overdominance \citep{landau2013dispersion}. A representative set of ROC curves is shown in Figure \ref{f:roc}. 
\begin{figure}[htbp]
...
\label{f:roc}
\end{figure}
The ROC curves are modest, e.g. for a false positive rate of 5\%, the best performing methods only achieved a true positive rate of just over 15\%. This is consistent with our expectation discussed in Section \ref{s:sim_data} due to the high signal-to-noise ratio in these simulated data. 
...
An alternative explanation for phenotypic heterosis is preferrential gene expression of alleles from certain parents for certain genes. Analyses of allele-specific expression \citep{bell2013rna,wei2013computational} could cmoplement the search for gene expression heterosis in the quest for the mechanism behind phenotypic heterosis.
