...
Following \cite{ji2014estimation}, we reparameterize the gene-variety mean structure into the gene-specific parental average ($\phi_g$), half-parental difference ($\alpha_g$), and hybrid difference ($\delta_g$). Specifically, if $V=3$ with $v=1,2$ indicate the two parental varieties and $v=3$ indicates the hybrid, then
\[ \phi_g = \frac{\mu_{g1}+\mu_{g2}}{2} \qquad \alpha_g = \frac{\mu_{g1}-\mu_{g2}}{2} \qquad \delta_g = \phi_g-\mu_{g3}. \]

We present a hierarchical model for the normalization factors, overdispersions, and gene-specific mean parameters. These normalization factors, parental averages, and overdispersion parameters are assumed to follow a normal distribution. 
\[ 
\gamma_{vi} \stackrel{ind}{\sim} N(0,\sigma_\gamma^2) \qquad 
\phi_g \stackrel{ind}{\sim} N(\eta_\phi, \sigma_\phi^2) \qquad 
\psi_g \stackrel{ind}{\sim} N(0,\sigma_\psi^2) \qquad 
\]
To allow for more shrinkage with the half-parental difference and hybrid difference, we use a Laplace (or double exponential prior) and thus implement a Bayesian LASSO \citep{park2008bayesian}. Thus
\[ 
\alpha_g \stackrel{ind}{\sim} La(\eta_\alpha, \sigma_\alpha) \qquad
