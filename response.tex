...
\response{We have provided a comparison of the computational time on the real data set using these methods. We have compared point estimates from edgeR to those using eBayes (laplace). }
...
\response{We now compare the estimated gene-specific parameters to the eBayes estimates in Figure 3. We have added a paragraph to the beginning of Section 2.2 to share our experience with a fully MCMC analysis which took too long and had poor mixing.}
...
\response{We now include a reference to Chen (2013), a Nature Reviews Genetics article entitled ``Genomic and epigenetic insights into the molecular bases of heterosis''. Even though scientists may have specific candidates. An exploratory analysis to suggest other genes seems useful.} 
